...
<?php
	...
	$cond = '';
	   if(isset($_POST['cond']) || isset($_POST['order'])):
		   	// if there is more then a condition implode it with AND
		   	 if(!empty($_POST['cond']) && is_array($_POST['cond'])){
		   	 	if(count($_POST['cond']) > 1 )
		   	 		$cond = implode(' and ', $_POST['cond']);
			   	 else
			   	 	$cond = $_POST['cond'][0];
		   	 }else{
		   	 ...
		   	 $order = $_POST['order'];
		   	 $products = $getFromP->get_products($cond, $order);
	   else:
...
<?php else: ?>
	...
	<div class="d-flex justify-content-center align-items-center flex-column container">
		...
		<h3 class="mt-4 mb-3 text-center text-muted font-weight-light"> Sorry ! no product was found for this filter </h3>
	</div>
